...
export function BookingFacetedFilter<T extends string | number>({
  ...
  const content = (
    <div className="flex flex-col" style={{ width: popoverWidth }}>
      {showSearch && (
      ...
      <ScrollArea className="max-h-[240px]" viewportClassName="p-1">
      ...
      <FilterTriggerButton icon={<CirclePlus size={16} />}>
        ...
        {selected.length > 0 ? (
          <>
            ...
            {selectedLabelOverride ? (
              <FilterBadge>{selectedLabelOverride}</FilterBadge>
          ...
          emptySelectionBadge && (
            <>
              <span className="mx-0.5 h-4 w-px bg-admin-border" />
              <FilterBadge>{emptySelectionBadge}</FilterBadge>
            </>
          )
        )}
      </FilterTriggerButton>
